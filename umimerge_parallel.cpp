...
string readWells(char *filename,vector<string> &wellList,vector<string> &shortWellList);
...
int main(int argc, char *argv[]){
 ...
 string wellTotalFile_mm=string(dge_dir)+"/"+string(sample_id)+".all.well_summary.dat";
  ...
  //allocate memory for total
	...
	unordered_set<string>* unknown_set_t=new unordered_set<string>[nThreads];
	unordered_set<string>* unknown_set_t_mm=new unordered_set<string>[nThreads]; 
 #pragma omp parallel for nthreads=nThreads
 for(int wellIndex=0;wellIndex<wellList.size();wellIndex++){
			int th=omp_get_thread_num();
  string well=shortWellList[wellIndex];
    ...
    if(items.size() > 19){				
	    ...
	    for (int k=0;k<split_loc.size()-1;k++){//extra ; at end so need the -1
				...
				if (aligned_id.substr(0,4) == "ERCC"){
					...
					uint64_t umi=wellIndex*wellBase+barcodeIndex*barcodeBase+barcodeBase-1;
					if(!best_hits_list.size()){
					...
					string umi=shortWellList[wellIndex] + barcode + aligned_id;
					fprintf(stderr,"%s\n",shortWellList[wellIndex].c_str());
					if(!best_hits_list.size()){
						count_assigned_unknown_reads++;
					 if(!unknown_set_t[th].count(aligned_id)){
					 	unknown_set_t[th].insert(aligned_id);
					 }
					...
					if(!unknown_set_t_mm[th].count(aligned_id)){
					 unknown_set_t_mm[th].insert(aligned_id);
					}
 ...
 fclose(fp);
 unordered_set<string> unknown_set,unknown_set_mm;
 for(int th=0;th<nThreads;th++){
		for (auto itr = unknown_set_t[th].begin(); itr != unknown_set_t[th].end(); ++itr)
   unknown_set.insert(*itr);
  for (auto itr = unknown_set_t_mm[th].begin(); itr != unknown_set_t_mm[th].end(); ++itr)
 ...
 delete[] unknown_set_t;  
 delete[] unknown_set_t_mm;
...
string readWells(char *filename,vector<string> &wellList,vector<string> &shortWellList){
	...
	while(fgets(line,sizeof(line),fp)){
	 ...
	 shortWellList.push_back(string(well));
	}
	fclose(fp);
	return string(id);	
}
